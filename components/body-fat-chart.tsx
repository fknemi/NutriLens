...
export default function BodyFatChart({
  ...
  // Guard against divide-by-zero when only one data point is passed.
  const xDenominator = Math.max(data.length - 1, 1);
  const toX = (i: number) => PAD_L + (i / xDenominator) * cW;
  ...
  return (
    ...
    >
      ...
      {/* Stat */}
      <View>
        <Text className="text-2xl font-semibold">{changeLabel}</Text>
        <Text className="text-base text-[#818181]">in this period</Text>
      </View>
      ...
      {chartWidth > 0 && hasData ? (
        ...
        <View style={{ height: H + PAD_B }} className="items-center justify-center">
          <Text className="text-sm text-[#bbb]">No data for this period</Text>
        </View>
      ) : null}
    </View>
  );
