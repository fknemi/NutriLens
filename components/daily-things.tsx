...
function useMealMacros(category: MealCategory, gender: Gender) {
  ...
  // Use getPlanForDate to extract the snapshot map for today safely
  const planForToday = useMealsStore((state) => state.getPlanForDate(today));
  const categoryMeals = planForToday[category] ? Object.values(planForToday[category]) : [];
  ...
  categoryMeals.forEach((meal) => {
    const cals = meal.nutrition_summary?.calories ?? meal.nutrition?.per_serving?.calories ?? 0;
    const protein = meal.nutrition_summary?.protein_g ?? meal.nutrition?.per_serving?.protein_g ?? 0;
    const carbs = meal.nutrition_summary?.carbohydrates_g ?? meal.nutrition?.per_serving?.carbohydrates_g ?? 0;
    const fats = meal.nutrition_summary?.fat_g ?? meal.nutrition?.per_serving?.fat_g ?? 0;
  ...
  const GOALS = {
    male: { protein: 150, carbs: 300, fats: 80 },
    female: { protein: 120, carbs: 220, fats: 60 },
  };

  const dailyGoals = GOALS[gender] || GOALS.male;
...
function DailyThings() {
  ...
  const [activeMealCategory, setActiveMealCategory] = useState<MealCategory | null>(null);
  const { dailyVolume, dailyGoal } = useHydrationStore();
  
  // Sync today's snapshot when the component mounts to freeze initial baselines
  const syncTodaySnapshot = useMealsStore((state) => state.syncTodaySnapshot);
  useEffect(() => {
    syncTodaySnapshot();
  }, [syncTodaySnapshot]);

  // Pass an explicit gender fallback argument here if needed, e.g., "male"
  const breakfastMacros = useMealMacros("breakfast", "male");
  const lunchMacros = useMealMacros("lunch", "male");
  const dinnerMacros = useMealMacros("dinner", "male");
  ...
  const totalCalories = activities.reduce((sum, item) => sum + item.caloriesBurned, 0);

  const activityName = activities.length > 0 ? "Daily Total" : "No Activity";
  const activityType = activities.length > 0 ? `${activities.length} sessions logged` : "Log a workout";
