...
// `name` stays as a prop (it's display-only, not store state).
// Volume/goal/progress and the add action now all come from useHydrationStore,
// so the parent no longer needs to pass waterVolumeReached / goal / onAdd.
function WaterCard({ name }) {
  const [modalVisible, setModalVisible] = useState(false);

  const dailyVolumeMl = useHydrationStore((state) => state.dailyVolume);
  const dailyGoalMl = useHydrationStore((state) => state.dailyGoal);
  const addWater = useHydrationStore((state) => state.addWater);
  const checkAndResetDay = useHydrationStore((state) => state.checkAndResetDay);

  // checkAndResetDay() is also called inside addWater(), but calling it here
  // too means the card shows 0 immediately on a fresh day even before the
  // user adds water (e.g. if they just open the app after midnight).
  useState(() => {
    checkAndResetDay();
  });

  // Store keeps everything in ml; WaterCard's formatVolume/progress math
  // expects litres, so convert at the boundary rather than touching the store's units.
  const waterVolumeReached = dailyVolumeMl / 1000;
  const goal = dailyGoalMl / 1000;

  const progress = goal > 0 ? (waterVolumeReached / goal) * 100 : 0;
  const { display, unit } = formatVolume(waterVolumeReached);

  const handleQuickAdd = (litres) => {
    addWater(litres * 1000); // store expects ml
